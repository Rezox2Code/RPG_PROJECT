...
class RPGUI:
    def __init__(self, root):
        ...
        self.root.configure(bg="#1E1E2F")  # fond sombre style RPG

        self.image_folder = "images"  # dossier des images
    ...
    "Dragon": {
        ...
        return [Character(name, descriptions[name]) for name in descriptions]

    def setup_ui(self):
        self.center_frame = tk.Frame(self.root, bg="#2A2A3D")
        self.center_frame.place(relx=0.5, rely=0.4, anchor="center", width=400, height=350)
        ...
        self.button_validate = tk.Button(self.center_frame, text="Valider", bg="#2ECC71", font=("Arial", 12, "bold"), command=self.validate)
        ...
        self.result = tk.Label(self.root, text="", font=("Arial", 12, "bold"), fg="#00BFFF", bg="#1E1E2F")
        self.result.pack(side="bottom", pady=20)

    def load_image(self, char_name):
        filename = f"{char_name}.png"
        filepath = os.path.join(self.image_folder, filename)

        if not os.path.exists(filepath):
            filepath = os.path.join(self.image_folder, "default.png")  # image par défaut si manquante

        try:
            img = Image.open(filepath) # adapte la taille
    ...
    def update_display(self):
        ...
        if img:
            ...
            self.label_image.image = img  # empêcher le garbage collector
        else:
    ...
    def validate(self):
        ...
        self.result.config(text=f"Tu as choisi : {chosen} | Spécilisation : {self.spec_var.get()}")
